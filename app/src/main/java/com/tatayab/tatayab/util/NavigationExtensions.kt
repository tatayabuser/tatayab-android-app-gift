...
fun NavigationView.setupWithNavController(
    ...
    navGraphIds.forEachIndexed { index, navGraphId ->
        ...
        // Attach or detach nav host fragment depending on whether it's the selected item.
        if (this.checkedItem?.itemId == graphId) {
            // Update livedata with the selected graph
            selectedNavController.value = navHostFragment.navController
            attachNavHostFragment(fragmentManager, navHostFragment, index == 0)
        } else {
    ...
    // Now connect selecting an item with swapping Fragments
    var selectedItemTag = graphIdToTagMap[this.checkedItem!!.itemId]
    val firstFragmentTag = graphIdToTagMap[firstFragmentGraphId]
    var isOnFirstFragment = selectedItemTag == firstFragmentTag
...
private fun NavigationView.setupDeepLinks(
    ...
    navGraphIds.forEachIndexed { index, navGraphId ->
        ...
        // Handle Intent
        if (navHostFragment.navController.handleDeepLink(intent)
            && checkedItem?.itemId != navHostFragment.navController.graph.id
        ) {

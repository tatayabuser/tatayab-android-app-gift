...
import com.tatayab.model.CartMap
...
import com.tatayab.tatayab.main.cart.CartAdapter2
import com.tatayab.tatayab.main.cart.CartFragmentArgs
import com.tatayab.tatayab.main.cart.CartFragmentDirections
import com.tatayab.tatayab.main.cart.GiftAdapter
import com.tatayab.tatayab.main.cart.GiftAdapter2
...
import kotlinx.android.synthetic.main.fragment_cart.animationView
import kotlinx.android.synthetic.main.fragment_cart.appTallbar
import kotlinx.android.synthetic.main.fragment_cart.cart_group
import kotlinx.android.synthetic.main.fragment_cart.deliveryAsGiftTextView
import kotlinx.android.synthetic.main.fragment_cart.giftButtonSepretor
import kotlinx.android.synthetic.main.fragment_cart.giftViewLayout
import kotlinx.android.synthetic.main.fragment_cart.layout_empty_cart
import kotlinx.android.synthetic.main.fragment_cart.layout_order_status
import kotlinx.android.synthetic.main.fragment_cart.layout_sub_total
import kotlinx.android.synthetic.main.fragment_cart.mainGiftView
import kotlinx.android.synthetic.main.fragment_cart.nestedScrollView
import kotlinx.android.synthetic.main.fragment_cart.refresh
import kotlinx.android.synthetic.main.fragment_cart.rv_cart
import kotlinx.android.synthetic.main.fragment_cart.upSell_message
import kotlinx.android.synthetic.main.fragment_cart.up_sell_text
import kotlinx.android.synthetic.main.gift_view.closeButton
import kotlinx.android.synthetic.main.gift_view.giftMessageEditText
import kotlinx.android.synthetic.main.gift_view.giftProductView
import kotlinx.android.synthetic.main.gift_view.giftProductView1
import kotlinx.android.synthetic.main.gift_view.giftProductView2
import kotlinx.android.synthetic.main.gift_view.giftProductView3
import kotlinx.android.synthetic.main.gift_view.giftProductView4
import kotlinx.android.synthetic.main.gift_view.giftRecyclerView
import kotlinx.android.synthetic.main.gift_view.giftRecyclerView1
import kotlinx.android.synthetic.main.gift_view.giftRecyclerView2
import kotlinx.android.synthetic.main.gift_view.giftScrollView
import kotlinx.android.synthetic.main.gift_view.giftTitleTextView
import kotlinx.android.synthetic.main.gift_view.giftTitleTextView1
import kotlinx.android.synthetic.main.gift_view.giftTitleTextView2
import kotlinx.android.synthetic.main.gift_view.giftTitleTextView3
import kotlinx.android.synthetic.main.gift_view.giftTitleTextView4
import kotlinx.android.synthetic.main.gift_view.receiverNameEditText
import kotlinx.android.synthetic.main.gift_view.removeGiftButton
import kotlinx.android.synthetic.main.gift_view.senderNameEditText
import kotlinx.android.synthetic.main.gift_view.submitGiftButton
import kotlinx.android.synthetic.main.layout_empty_cart.btn_view_products
import kotlinx.android.synthetic.main.layout_order_status.btn_continue_shopping
import kotlinx.android.synthetic.main.layout_order_status.cashBackGiftLottie
import kotlinx.android.synthetic.main.layout_order_status.cashBackMessageTextView
import kotlinx.android.synthetic.main.layout_order_status.cashBackView
import kotlinx.android.synthetic.main.layout_order_status.checkBalanceButton
import kotlinx.android.synthetic.main.layout_order_status.closeCashBackViewButton
import kotlinx.android.synthetic.main.layout_order_status.deliveryDateTextView
import kotlinx.android.synthetic.main.layout_order_status.deliveryDateView
import kotlinx.android.synthetic.main.layout_order_status.delivery_message
import kotlinx.android.synthetic.main.layout_order_status.group_knet
import kotlinx.android.synthetic.main.layout_order_status.tv_order_id
import kotlinx.android.synthetic.main.layout_order_status.tv_reference_no
import kotlinx.android.synthetic.main.layout_order_status.tv_transaction_id
import kotlinx.android.synthetic.main.layout_sub_total_2.btn_continue_payment
import kotlinx.android.synthetic.main.layout_sub_total.notes
import kotlinx.android.synthetic.main.layout_sub_total_2.btn_skip
import kotlinx.android.synthetic.main.toolbar_main_default.shareCartButton
import javax.inject.Inject


class GiftOptionsFragment : BaseFragment2(), OnCartListener, OnUpdateAmountListener,
    NavigationResult,
    OnGiftListener {
    ...
    private var selectedGiftProductc: ProductX? = null
    private var selectedGiftProductIDc: String? = null
    private var oldSelectedGiftProductIDc: String? = ""
    ...
    override fun layoutId(): Int {
        return R.layout.fragment_gift_options
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mGiftAdapter2 = GiftAdapter2(this, viewModel.getDecimalNumbers())
        mGiftAdapter3 = GiftAdapter(this, viewModel.getDecimalNumbers())
        mGiftAdapter4 = GiftAdapter(this, viewModel.getDecimalNumbers())
        ...
        cartAdapter = CartAdapter2(this, this,mainViewModel?.getCountryCode())

        mainViewModel?.ENABLE_GRAPH_QUERIES_CALLS = Constants.ENABLE_GRAPH_QUERIES_CALLS
        mainViewModel?.getMyWallet()

        mainViewModel?.getProductGiftsLiveData()!!.observe(this, Observer {
            when (it.status) {
                ResourceState.ERROR -> {
                    Log.e("TAG", "mgiftonCreate: ${it.message}")
                }
                ResourceState.SUCCESS -> {
                    it.data?.let {
                        it?.second?.let {
                            Log.d("TAG", "MonCreate: ${it.products}")
                ...
                ResourceState.SUCCESS -> {
                    ...
                    if (it.data?.success == 1) {
                        ...
                        if (!senderNameEditText.text.toString().isNullOrEmpty()) {
                            ...
                            MemoryCacheManager.mGiftModelConstant = giftModel
                        } else {
                            mGiftModelCached = null
                            MemoryCacheManager.mGiftModelConstant = null
                        }
                        selectedGiftProduct = null
                        nextSelectedGiftId = null
                        selectedGiftProductID = null
                        selectedGiftProductc = null
                        nextSelectedGiftIdc = null
                        selectedGiftProductIDc = null
                        ...
                        if (!paymentSuccess) viewModel?.getCart(Constants.ENABLE_GRAPH_QUERIES_CALLS)
                    } else {
                        if (!paymentSuccess) {
                            showCustomTopMessage(
                                it?.data?.msg + "",
        ...
        viewModel.getCartLiveData.observe(this, Observer { it ->
            ...
            when (it.status) {
                ResourceState.ERROR -> {
                    ...
                    fireBaseLog.isGuest = viewModel.isUserLogin(Constants.ENABLE_GRAPH_QUERIES_CALLS)
...
//                    setProgress(View.GONE)
                    ...
                    it.data?.let { cart ->
                        ...
                        if (cart.second.products!!.isNotEmpty()) {
                            ...
                            InsiderManager.ITEMS_PURCHASED = cart.second.products
...
//                    setProgress(View.GONE)
                    ...
                    if (selectedGiftProduct != null) {
                        viewModel?.addGiftToCart(
                            ...
                            selectedGiftProductc!!.product_id,
                            senderNameEditText.text.toString(),
                            receiverNameEditText.text.toString(),
                            giftMessageEditText.text.toString(),
                            productGraphID,
                        ...
                        if (senderNameEditText.text.toString().isNullOrEmpty()) {
                            ...
                            MemoryCacheManager.mGiftModelConstant = null
                            comeFromDelete = true
                            if (Constants.ENABLE_GRAPH_QUERIES_CALLS.not()) viewModel?.addGiftToCart(
                                "",
                                "",
                                "",
                                "",
                            ...
                            if (oldSelectedGiftProductID.isNullOrEmpty()) {
                                ...
                                if (oldSelectedGiftProductID.isNullOrBlank()) oldSelectedGiftProductID =
                                    ""
                                if(oldSelectedGiftProductIDc.isNullOrBlank()) oldSelectedGiftProductIDc=""
                                viewModel?.addGiftToCart(
                                    oldSelectedGiftProductID!!,
                                    oldSelectedGiftProductIDc!!,
                                    senderNameEditText.text.toString(),
                                    receiverNameEditText.text.toString(),
                                    giftMessageEditText.text.toString(),
                                    productGraphID,
                        ...
                        selectedGiftProductIDc = null
                        oldSelectedGiftProductIDc = null
...
//                    setProgress
        ...
        mainViewModel.getWalletLiveData.observe(this, Observer {
            when (it.status) {
                ResourceState.ERROR -> {
                    MemoryCacheManager.walletCredit = 0f
                }
                ResourceState.SUCCESS -> {
                    try {
                        it?.data?.mWalletData?.avalAmount?.let {
                            MemoryCacheManager.walletCredit = it.toFloat()
                        } ?: run {
                            MemoryCacheManager.walletCredit = 0f
                        }
                    } catch (e: java.lang.Exception) {
    ...
    private fun checkGiftMessage(cartResponse: GetOrdersFromCartResponse) {
        cartResponse?.let {
            if (!it?.gift_message.isNullOrEmpty()) {
                ...
                MemoryCacheManager.mGiftModelConstant = mGiftModel
            }
            updateGiftText()
        }
    ...
    override fun onResume() {
        ...
        mGiftModelCached = null
        if (MemoryCacheManager.mGiftModelConstant != null) {
            selectProductUIDConnectToGift = MemoryCacheManager.mGiftModelConstant?.productID.toString()
            Log.d("TAG", "MDataonResume: ${selectProductUIDConnectToGift}")
        }
        MemoryCacheManager.mGiftModelConstant = null
    ...
    private fun trackOrderByInsider() {
        if (!InsiderManager.ITEMS_PURCHASED.isNullOrEmpty() && InsiderManager.ITEMS_PURCHASED!!.isNotEmpty() && !orderId.isNullOrEmpty()) {
            for (item in InsiderManager.ITEMS_PURCHASED!!) {
                try {
                    item.let {
            ...
            InsiderManager.ITEMS_PURCHASED = null
    ...
    private fun showCartContent(visibility: Int) {
        ...
        layout_sub_total.visibility = View.VISIBLE
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (FirebaseConfigUtil.SHARE_CART_ENABLED && !Constants.ENABLE_GRAPH_QUERIES_CALLS) {
        ...
        if (!orderId.isNullOrEmpty() && paymentSuccess!!) {
            ...
            try {
                ...
                Smartlook.trackCustomEvent(com.tatayab.tatayab.util.Constants.PLACE_ORDER)

            } catch (e: Exception) {
                e.printStackTrace()
            }
            arguments?.putString("orderId", null)
            if (!Constants.ENABLE_GRAPH_QUERIES_CALLS) viewModel?.checkCashBack(orderId!!)
        } else {
            viewModel.getCart(Constants.ENABLE_GRAPH_QUERIES_CALLS)
        }
    ...
    private fun setupGiftList() {
        productGistResponse?.let {
            ...
            if (it?.products.isNullOrEmpty()) {
                ...
                giftTitleTextView.visibility = View.GONE
                ...
                for (item in it?.products!!) {
                    if (item?.category == "BOX") {
                        ...
                        giftTitleTextView.visibility = View.VISIBLE

                        giftTitleTextView.text = category

                        mGiftAdapter.setData(
                            viewModel.getCurrencyCode(),
                            it?.products?.filter { productX -> productX?.category == "BOX" } as ArrayList<ProductX>,
                            oldSelectedGiftProductID
                        )
    ...
    private fun setupGiftList1() {
        productGistResponse?.let {
            ...
            if (it?.products.isNullOrEmpty()) {
                ...
                giftTitleTextView1.visibility = View.GONE
                giftProductsList1 = null
            } else {
                for (item in it?.products!!) {
                    if (item?.category == "Bouquet") {
                        category1 = item?.category
                        giftProductView1.visibility = View.VISIBLE
                        giftTitleTextView1.visibility = View.VISIBLE

                        giftTitleTextView1.text = category1
                        mGiftAdapter1.setData(
                            viewModel.getCurrencyCode(),
                            it?.products?.filter { productX -> productX?.category == "Bouquet" } as ArrayList<ProductX>,
                            oldSelectedGiftProductID
                        )
                        giftProductsList1 = it?.products as ArrayList<ProductX>
    ...
    private fun setupGiftList2() {
        productGistResponse?.let {
            ...
            if (it?.products.isNullOrEmpty()) {
                ...
                giftTitleTextView2.visibility = View.GONE
                giftProductsList2 = null
            } else {
                for(item in it?.products!!){
                    if(item?.category == "giftcards"){
                        category2=item?.category
                        giftProductView2.visibility = View.VISIBLE
                        giftTitleTextView2.visibility = View.VISIBLE
                    }
                }
                ...
                mGiftAdapter2.setData(
                    viewModel.getCurrencyCode(),
                    it?.products?.filter { productX -> productX?.category == "giftcards" } as ArrayList<ProductX>, oldSelectedGiftProductIDc
                )
                giftProductsList2 = it?.products as ArrayList<ProductX>
                ...
                if (currentGiftItemIdc!! > 0) {

                    nextSelectedGiftIdc = currentGiftItemIdc.toString()
                    var giftItem = getGiftItemIfExist(currentGiftItemIdc)
                //var giftItem = getGiftItemIfExist(nextSelectedGiftIdc?.toInt())
                    try {
                        if (giftItem != null && !giftItem.product_id.equals(cartAdapter?.getGiftIDIfExist())) {
                            cartAdapter.addGiftItem(giftItem)
                        }
    ...
    var giftProductsList3: ArrayList<ProductX>? = null
    var category3:String?=""
    private fun setupGiftList3() {
        productGistResponse?.let {
            ...
            if (it?.products.isNullOrEmpty()) {
                //if (giftProductsList1.isNullOrEmpty()) {
                giftProductView3.visibility = View.GONE
                giftTitleTextView3.visibility = View.GONE
                giftProductsList3 = null
            } else {
                for(item in it?.products!!){
                    if(item?.category == "CHOCOLATE"){
                        category3=item?.category
                        giftProductView3.visibility = View.VISIBLE
                        giftTitleTextView3.visibility = View.VISIBLE

                        giftTitleTextView3.text=category3
                        mGiftAdapter3.setData(
                            viewModel.getCurrencyCode(),
                            it?.products?.filter { productX -> productX?.category == "CHOCOLATE" } as ArrayList<ProductX>, oldSelectedGiftProductID
                        )
                        giftProductsList3 = it?.products as ArrayList<ProductX>
    ...
    var giftProductsList4: ArrayList<ProductX>? = null
    var category4:String?=""
    private fun setupGiftList4() {
        productGistResponse?.let {
            ...
            if (it?.products.isNullOrEmpty()) {
                //if (giftProductsList1.isNullOrEmpty()) {
                giftProductView4.visibility = View.GONE
                giftTitleTextView4.visibility = View.GONE
                giftProductsList4 = null
            } else {
                for(item in it?.products!!){
                    if(item?.category == "NA"){
                        category4=item?.category
                        giftProductView4.visibility = View.VISIBLE
                        giftTitleTextView4.visibility = View.VISIBLE

                        giftTitleTextView4.text=category4
                        mGiftAdapter4.setData(
                            viewModel.getCurrencyCode(),
                            it?.products?.filter { productX -> productX?.category == "NA" } as ArrayList<ProductX>, oldSelectedGiftProductID
                        )
                        giftProductsList4 = it?.products as ArrayList<ProductX>
    ...
    private fun intComponent() {
        try {
            ...
            btn_continue_payment.setSafeOnClickListener {
               ...
               // findNavController().navigate(R.id.action_giftOptionsFragment_to_paymentFragment)
                var args=Bundle()
                args.putBoolean("GiftOptions",true)
                findNavController().navigate(R.id.action_giftOptionsFragment_to_checkoutFragment,args)
               // findNavController().navigate(R.id.action_giftOptionsFragment_to_destination_addresses,args)
            }

            btn_skip.setSafeOnClickListener {
               // viewModel.continueToPayment
              /*  var args=Bundle()
                args.putBoolean("GiftOptions",true)
                findNavController().navigate(R.id.action_giftOptionsFragment_to_destination_addresses,args)*/
                //findNavController().navigate(R.id.action_giftOptionsFragment_to_checkoutFragment)
            ...
            submitGiftButton.setOnClickListener {
               // Log.d("TAG", "MintComponent: ${selectProductUIDConnectToGift}")
                try {
                    Log.d("TAG", "MGift: ${mwrap_id} - ${mcard_id}")
                    if (!senderNameEditText.isValid(requireContext())
                        ...
                        comeFromDelete = true
                        Log.d("TAG", "mGiftintComponent: ${productGraphID}")
                        ...
                        if (!nextSelectedGiftId.isNullOrEmpty() && !unSelectedProductID.equals(
                            ...
                            viewModel.addGiftToCart(
                                //selectedGiftProduct!!.product_id,
                                //selectedGiftProductc!!.product_id2,
                                mwrap_id!!,
                                mcard_id!!,
                                senderNameEditText.text.toString(),
                                receiverNameEditText.text.toString(),
                        ...
                        } else if (selectedGiftProduct != null && selectedGiftProductc != null) {
                            viewModel.addGiftToCart(
                               // selectedGiftProduct!!.product_id,
                              //  selectedGiftProductc!!.product_id2,
                                mwrap_id!!,
                        ...
                        btn_continue_payment.isEnabled=true
...
//                    }
    ...
    private fun notifyHomeWithChanges(prductID: String, newStatues: Boolean) {
        val intent = Intent(com.tatayab.tatayab.util.Constants.CHANGE_WISHLIST_CASE)
        intent.putExtra(com.tatayab.tatayab.util.Constants.PRODUCT_ID, prductID)
        intent.putExtra(com.tatayab.tatayab.util.Constants.NEW_STATEUES, newStatues)
...
//            }
    ...
    override fun updateGift(mProductX: ProductX) {
        mProductX?.let {
            ...
            /*nextSelectedGiftIdc = if (mProductX?.product_id2.isNullOrBlank()
                    .not()
            ) mProductX?.product_id2 else ""*/
        }
